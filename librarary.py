...
def save_user():
    ...
    if 'loggedin' in session:        
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        if session['role']=='admin'and request.method == 'POST' and 'role' in request.form and 'first_name' in request.form and 'last_name' in request.form and 'email' in request.form :
...
def delete_user():
    if 'loggedin' in session:
        deleteUserId = request.args.get('id')
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
...
def save_issue_book():
    if 'loggedin' in session:
        ...
        if  session['role']=='admin' and  request.method == 'POST' and 'book' in request.form and 'users' in request.form and 'expected_return_date' in request.form and 'return_date' in request.form and 'status' in request.form:
...
def saveCategory():
    if 'loggedin' in session:  
        ...
        if  session['role']=='admin' and request.method == 'POST' and 'name' in request.form and 'status' in request.form:
